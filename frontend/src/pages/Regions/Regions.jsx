    ...
    async function loadRegions() {
      try {
        ...
        const response = await fetch(
          `${API_URL}/api/v1/regions?lang=${apiLang}`,
          {
  ...
  return (
    <main className="region">
      ...
      >
        ...
        {error && (
          <p className="region__status region__status--error">
            {error}
          </p>
        )}
        ...
        {!loading && !error && (
          <div className="region__content">
            {regions.length > 0 ? (
              <section
                className="region__grid"
                aria-label={t.allRegionsAria}
              >
                {regions.map((region) => {
                  const id = getRegionId(region);
                  const banner =
                    getRegionBanner(region);
                  const crest =
                    getRegionCrest(region);
                  const description =
                    ...
                    >
                      <span className="region__banner">
                        {banner ? (
                          ...
                          <span className="region__fallback">
                            {name}
                          </span>
                        )}

                        <span
                          className="region__shade"
                          aria-hidden="true"
                        />
                      </span>
                      ...
                      <span className="region__body">
                        <span className="region__head">
                          <span className="region__crestBox">
                            {crest ? (
                              ...
                              <span
                                className="region__crestFallback"
                                aria-hidden="true"
                              >
                                {String(
                                  name || "?"
                          ...
                          <span className="region__nameBox">
                            ...
                            {region.title && (
                              <span className="region__tagline">
                                {region.title}
                              </span>
                            )}
                          </span>
                        ...
                        {description && (
                          <span className="region__desc">
                            {description}
                          </span>
                        )}

                        <span className="region__bottom">
                          <span className="region__cta">
                            {t.viewRegion}
                          </span>
